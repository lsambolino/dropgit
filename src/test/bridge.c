...
move(struct my_ring* src, struct my_ring* dst, u_int limit)
{
	...
	u_int si = src->begin;
	u_int di = dst->begin;
    const char* msg = (src->queueid & NETMAP_SW_RING) ? "host->net" : "net->host";
...
main(int argc, char** argv)
{
    ...
    D("Ready to go, %s 0x%x/%d <-> %s 0x%x/%d.", me[0].ifname, me[0].queueid, me[0].nifp->ni_rx_rings, me[1].ifname, me[1].queueid, me[1].nifp->ni_rx_rings);
